...
export type Connections = number[][];
//   in1   in2   sh3
//  ----------------------
// [ W14   W24   W34 ] out4
// [ W15   W25   W35 ] out5
 ...
 */
export function forwards(l1: Neuron[], c: Connections, l2: Neuron[], activator: (x: number) => number)
{
	for (let o = 0; o < l2.length; o++) {
		...
		if (c[o].length > 0) no.value = activator(value);
	}
}
/**
 ...
 */
export function findError(l1: Neuron[], c: Connections, l2: Neuron[])
{
	for (let i = 0; i < l1.length; i++) {
		const ni = l1[i];
		ni.error = 0;
 ...
 */
export function backwards(l1: Neuron[], c: Connections, l2: Neuron[], n: number, d: (y: number) => number)
{
	for (let o = 0; o < l2.length; o++) {
		const no = l2[o];
		for (let i = 0; i < l1.length; i++) {
			const ni = l1[i];
			if (c[o]?.[i] == undefined) continue;
