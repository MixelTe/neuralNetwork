...
export function start() {
    ...
    function loadModel(modeSave) {
        network.Activator = modeSave.fn;
    ...
    function updateStats() {
        if (last_answ.length == 0)
            return;
        const indexes = (len) => new Array(len).fill(0).map((_, i) => i);
        const indexOfMax = (nums) => nums.indexOf(Math.max(...nums));
        const sum = (nums) => nums.reduce((c, v) => c + (typeof v == "boolean" ? (v ? 1 : 0) : v), 0);
    ...
    ;
    function sum(nums) {
        return nums.reduce((c, v) => c + (typeof v == "boolean" ? (v ? 1 : 0) : v), 0);
    }
